...
<div class="col-md-3">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
         <a href="{{route('branch1.history',['add'=>$prov->add])}}" type="button" class="btn btn-info text-light">see detail</a>
      </td>
      <td>
        <a href="{{route('branch.edit',['id'=>$prov->id])}}" type="button" class="btn btn-info text-light">edit</a>
        <button  type="button" class="btn btn-danger text-light" onclick="deleteAgent({{ $prov->id }})">delete</button>
                    <form id="delete-form-{{ $prov->id }}" action="{{ route('branch.destroy',$prov->id) }}" method="POST" style="display: none;">
                                                    {{ csrf_field() }}
