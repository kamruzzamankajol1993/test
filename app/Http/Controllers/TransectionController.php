...
class TransectionController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if($status == "paid" ){
          $details= Sendmoney::where('status','=',$status)->whereBetween('date', array($from_date, $to_date)) ->get();  
        }else{
         $details= Sendmoney::where('status','=',$status)->whereBetween('date', array($from_date, $to_date)) ->get();   
        }
    ...
    public function searchreceive(Request $request)
    {
        ...
        $from_date = $request->input('from');
        $to_date = $request->input('to');
        $reciver_nidnumber=$request->input('reciver_nidnumber');
       
    $details= Receivemoney::where('reciver_nidnumber','=',$reciver_nidnumber)->whereBetween('date', array($from_date, $to_date)) ->get();  
