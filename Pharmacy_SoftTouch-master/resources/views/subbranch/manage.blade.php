...
<div class="col-md-3">
  ...
  <thead>
    <tr>
      ...
      <th scope="col">Sender GSM</th>
      <th scope="col">Receiver Name</th>
      <th scope="col">Description</th>
      <th scope="col">Receiver GSM</th>
  ...
  <tbody>
    ...
    @if(Session::get('customerId') == $prov->user_id)
    <tr>
      <th scope="row">{{ $i++ }}</th>
      <td>{{$prov->sender_name}}</td>
      <td>{{$prov->sender_nid}}</td>
      <td>{{$prov->sender_number}}</td>
      <td>{{$prov->receiver_name}}</td>
      
      <td>{{$prov->receiver_nid}}</td>
      <td>{{$prov->receiver_number}}</td>
      
      <td>
         <a href="{{route('subbranch.edit',$prov->id)}}" type="button" class="btn btn-info text-light">edit</a>
       <!-- <button  type="button" class="btn btn-danger text-light" onclick="deleteAgent({{ $prov->id }})">delete</button>-->
                    <form id="delete-form-{{ $prov->id }}" action="{{ route('subbranch.destroy',$prov->id) }}" method="POST" style="display: none;">
                                                    {{ csrf_field() }}
